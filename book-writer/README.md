...
An AI agent that writes entire books overnight using local LLMs via [Ollama](https://ollama.com/). Provide a TOC, and wake up to a completed book.

[Korean Language Support $\rightarrow$ README_KR.md]
...
Table of Contents (JSON / YAML / text)
    ...
    ┌─────────────────────────────────────────┐
    │  5. Publisher                            │
    │     → Combine all chapters into PDF     │
    │     → git commit + push (always)        │
    └─────────────────────────────────────────┘
```
...
| **Publisher** | Converts all chapter Markdown files into a single PDF book (not an LLM agent) | — |
...
```bash
python run_book.py --toc my-book-toc.json --model gemma4:31b --agents outline,writer,reviewer,finalizer,publisher
```

**Publish existing chapters to PDF only (no LLM needed)**:

```bash
python run_book.py --toc my-book-toc.json --agents publisher --output-dir ./my-book
```
...
Required:
  ...
  --model MODEL          Ollama model name (required when running LLM agents, not needed for --agents publisher)
...
Options:
  ...
  --agents STAGES        Pipeline stages, comma-separated (default: outline,writer,reviewer,finalizer)
                         Add "publisher" to generate a combined PDF book after writing.
...
├── mastering-python.pdf              ← generated by publisher
├── .progress.json
└── book-writer.log
```
