#!/usr/bin/env python3
"""Tiny-jarvis runner. Autonomous AI software engineering team."""
...
def ensure_labels(config: Config) -> None:
    ...
    role_colors = {
        "pm": "0e8a16", "architect": "1d76db",
        "backend": "5319e7", "qa": "fbca04",
    }
...
This is Sprint 0 of the tiny-jarvis project. No product has been defined yet.

### PM: Your Mission

You are the Product Manager. The team builds a **Python CLI personal AI agent** with
Telegram message scheduling powered by a local Gemma model.

#### The Product Vision (Pre-defined)

**Tiny Jarvis** is a local personal AI agent that:
1. Takes natural language commands like "Tell Jisoo tomorrow at 9 AM that I finished the report"
...
- Telethon for Telegram userbot messaging
- OpenAI-compatible client for local Gemma via Ollama
- python-dotenv for configuration

#### Step 1: Write the Product Spec
1. Write `docs/vision.md` with the product description above
2. Write `docs/product-spec.md` with detailed module requirements:
   - Config loader (tools/config.py)
   ...
   - Main CLI (main.py)
   - Run scheduler (run_scheduler.py)
   - Logging tool (tools/logging_tool.py)
   - Tests for each module

#### Step 2: Create Initial Issues
1. Create an issue for **Architect** (label: role:architect): Design system architecture
   and initialize the Python project skeleton
2. Write `work_plan.json` — only PM runs this cycle

#### Safety Rules (Must Be in Product Spec)
- No mass messaging. One command = one message.
...
- Local-first: runs on developer's machine, no cloud deployment needed
- Must work with `python main.py` and `python run_scheduler.py`
- All tests must pass with `pytest`
"""
    subprocess.run(
        ["gh", "issue", "create",
         "--title", "[GENESIS] Sprint 0 — Define Product Vision & Spec",
         "--body", bootstrap_body,
...
async def main() -> None:
    ...
    logger.info("=" * 60)
    logger.info("  TINY-JARVIS — Autonomous AI Engineering Team")
    ...
    logger.info("  Agents: %s", ", ".join(config.agent_roles))
    ...
    cycle_number = 1
    logger.info("Tiny-jarvis is online. Press Ctrl+C to shut down.")
    ...
    logger.info("Tiny-jarvis shut down after %d cycles. Goodbye!", cycle_number - 1)
