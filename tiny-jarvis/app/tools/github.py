"""GitHub CLI wrapper tools for tiny-jarvis agents."""
...
def list_open_issues(label: str = "", limit: int = 20) -> str:
    """List open issues in the product repository.

    Args:
        label: Optional label to filter by (e.g., 'role:backend', 'P0-critical').
...
def create_issue(title: str, body: str, labels: str = "") -> str:
    """Create a new issue in the product repository.
    ...
    Args:
        ...
        labels: Comma-separated labels (e.g., 'role:backend,P1-high,status:todo').
...
def list_pull_requests(state: str = "open", limit: int = 10) -> str:
    """List pull requests in the product repository.
...
def view_pull_request(pr_number: int) -> str:
    ...
    Returns:
        JSON string with PR details including reviewDecision.
    """
    args = [
...
def create_pull_request(title: str, body: str, head_branch: str, base_branch: str = "") -> str:
    ...
    IMPORTANT: The branch MUST be pushed to the remote BEFORE calling this.
    Call git_commit_and_push first and verify it succeeded.
    ...
    if check.returncode != 0 or not check.stdout.strip():
        ...
        if push.returncode != 0:
            return json.dumps({"success": False, "error": f"Branch '{head_branch}' not on remote and push failed: {push.stderr.strip()}"})
        time.sleep(2)
...
def add_label_to_pr(pr_number: int, label: str) -> str:
    """Add a label to a pull request.

    Args:
        pr_number: The PR number.
        label: The label to add (e.g., 'qa:approved', 'qa:changes-requested').
    ...
    try:
        result = subprocess.run(
            ["gh", "api", f"repos/{_REPO}/issues/{pr_number}/labels",
             "--method", "POST", "--input", "-"],
        ...
        return json.dumps({"success": True, "label": label, "pr": pr_number})
    except Exception as e:
        return json.dumps({"success": False, "error": str(e)})


def remove_label_from_pr(pr_number: int, label: str) -> str:
    ...
    try:
        result = subprocess.run(
            ["gh", "api", f"repos/{_REPO}/issues/{pr_number}/labels/{encoded}",
             "--method", "DELETE"],
            capture_output=True, text=True, timeout=_TIMEOUT,
        )
        if result.returncode != 0 and "Not Found" not in result.stderr:
            return json.dumps({"success": False, "error": result.stderr.strip()})
        return json.dumps({"success": True, "removed": label, "pr": pr_number})
    except Exception as e:
        return json.dumps({"success": False, "error": str(e)})
